...
export const CosmicBackground = () => {
  ...
  useEffect(() => {
    ...
    systemPivot.add(saturnGroup); // Saturn is part of the main system

    const wireframeMaterial = new THREE.MeshBasicMaterial({ color: 0x9400D3, wireframe: true });
    
    const planetSegments = isMobile ? 16 : 32;
    const planetGeometry = new THREE.SphereGeometry(4, planetSegments, planetSegments);
    const planet = new THREE.Mesh(planetGeometry, wireframeMaterial);
    saturnGroup.add(planet);

    const ringSegments = isMobile ? 50 : 100;
    const ringGeometry = new THREE.TorusGeometry(6, 0.4, 16, ringSegments);
    const ring = new THREE.Mesh(ringGeometry, wireframeMaterial);
    ring.rotation.x = Math.PI / 2;
    saturnGroup.add(ring);
    
    camera.position.z = 12;

    // --- 2. CREATE THE STAR DUST AND ADD IT TO THE SATURN PIVOT ---
    // This makes the dust a child of the Saturn system, so it orbits around it.
    const dustColor = "#6A3093";
    ...
    const animate = () => {
        ...
        // Animate Saturn's own rotation
        saturnGroup.rotation.y += 0.005;

        // --- Animate the background star dust ---
