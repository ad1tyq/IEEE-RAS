...
export default function Navbar() {
  ...
  const navLinks = [
    { name: 'About', href: '/technovision3#about' },
    { name: 'Resources', href: '/technovision3/resources' },
    { name: 'Gallery', href: '/technovision3/gallery' },
  ];
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex space-x-8">
            {navLinks.map((link) => {
              ...
              const isActive = pathname === link.href || (link.name === 'About' && pathname === '/technovision3');
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`text-xl font-mono-pixel transition-colors ${
                    isActive ? 'text-cyan-500' : 'text-gray-300 hover:text-cyan-500'
                  }`}
                >
                  {link.name}
                </Link>
