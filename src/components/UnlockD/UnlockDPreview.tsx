...
/**
 * Props interface for the UnlockDPreview component
 */
interface UnlockDPreviewProps {
...
export default function UnlockDPreview({
  size = 80,
  className = '',
  speed = 1
}: UnlockDPreviewProps) {
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <div className="flex flex-col items-center justify-center space-y-0.5 mb-2">
          ...
          <div className="flex items-center justify-center space-x-0.5">
            {"UNLOCK".split("").map((letter, i) => (
              <motion.span
                key={`unlock-${i}`} custom={i} variants={letterVariants} initial="initial" animate="animate"
                className="inline-block text-cyan-400 font-bold"
                style={{
                  fontSize: size * 0.08,
                  textShadow: `0 0 ${size * 0.08}px rgba(34, 211, 238, 1), 0 0 ${size * 0.04}px rgba(34, 211, 238, 0.8)`,
          ...
          {/* 'D Text - Blue */}
          <div className="flex items-center justify-center space-x-0.5">
            {"'D".split("").map((letter, i) => (
              <motion.span
                key={`d-${i}`} custom={i + 6} variants={letterVariants} initial="initial" animate="animate"
                className="inline-block text-blue-400 font-bold"
                style={{
                  fontSize: size * 0.08,
                  textShadow: `0 0 ${size * 0.08}px rgba(96, 165, 250, 1), 0 0 ${size * 0.04}px rgba(96, 165, 250, 0.8)`,
                  fontFamily: 'monospace', letterSpacing: '1px'
                }}
              >
                {letter}
        ...
        >
          ...
          <div className="text-gray-400 tracking-wide" style={{ fontSize: size * 0.045, fontFamily: 'monospace' }}>CHALLENGE</div>
        </motion.div>
      </div>

      {/* Scanline Effects - Cyan/Blue */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(2)].map((_, i) => (
          <motion.div
            key={`scanline-${i}`}
            className="absolute w-full bg-gradient-to-r from-transparent via-cyan-500/30 to-transparent"
